...
public class ICICIStatementParser implements BankStatementParser {
    ...
    // Pattern for ICICI date at start: dd/MM/yyyy, dd-MM-yyyy, or dd-MMM-yyyy
    private static final Pattern DATE_PATTERN = Pattern.compile(
            "^(\\d{2}[/\\-](?:\\d{2}|[A-Za-z]{3})[/\\-]\\d{2,4})\\b");

    private static final Pattern AMOUNT_PATTERN = Pattern.compile("[0-9,]+\\.\\d{2}");

    private static final String[] IGNORE_KEYWORDS = {
        "ICICI BANK", "STATEMENT OF ACCOUNT", "Page ", "CLOSING BALANCE", "OPENING BALANCE",
        "Transaction Date", "Value Date", "Cheque No", "Transaction Remarks", "Amount (Rs.)",
        "Cr/Dr", "Balance (Rs.)", "Withdrawal", "Deposit", "S No"
    };
    ...
    public boolean canParse(String textHeader, String fullText) {
        ...
        return combined.contains("icici bank") || combined.contains("icicibank");
    }
    ...
    public List<RawTransactionRow> parse(String fullText) {
        ...
        for (String line : lines) {
            ...
            if (dateMatcher.find()) {
                ...
                current = parseIciciLine(trimmed, dateMatcher.group(1));
    ...
    private RawTransactionRow parseIciciLine(String line, String dateStr) {
