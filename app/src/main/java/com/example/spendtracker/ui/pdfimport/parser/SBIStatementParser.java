...
public class SBIStatementParser implements BankStatementParser {
    ...
    // Pattern for SBI dates: "05 Jan 2024", "05-Jan-24", "05/01/2024"
    private static final Pattern DATE_PATTERN = Pattern.compile(
            "^(\\d{2}(?:\\s+|-)[A-Za-z]{3}(?:\\s+|-)\\d{2,4}|\\d{2}/\\d{2}/\\d{2,4})\\b");

    private static final Pattern AMOUNT_PATTERN = Pattern.compile("[0-9,]+\\.\\d{2}");

    private static final String[] IGNORE_KEYWORDS = {
        "STATE BANK OF INDIA", "ACCOUNT STATEMENT", "Page ", "CLOSING BALANCE", "OPENING BALANCE",
    ...
    public boolean canParse(String textHeader, String fullText) {
        ...
        return combined.contains("state bank of india") || combined.contains("sbi ") || combined.contains("sbi\n") || combined.contains("sbi-");
    }
    ...
    public List<RawTransactionRow> parse(String fullText) {
        ...
        for (String line : lines) {
            ...
            if (dateMatcher.find()) {
                ...
                current = parseSbiLine(trimmed, dateMatcher.group(1));
    ...
    private RawTransactionRow parseSbiLine(String line, String dateStr) {
        ...
        boolean isCredit = line.toUpperCase().contains(" CR") || line.toLowerCase().contains("credited") ||
                           line.toLowerCase().contains("by transfer") || line.toLowerCase().contains("deposit");
